...
**AI 不是天生会截图**——截图能力来自浏览器自动化工具（MCP）。成稿流程开始时先判定当前环境，再决定工作模式：

- **有浏览器自动化工具**（Playwright MCP / chrome-devtools MCP / 任何能打开 URL 并截图的工具）→ 全自动：每个 Step 由 AI 打开原始 URL 截图。
- **无截图工具** → 降级为**人工供图**：每到一个需要截图的 Step，明确告诉用户"请把这一步的 Burp/浏览器截图保存为 `shots/step<N>_<说明>.png`"，用户放好后嵌入对应位置。**禁止**因为没工具就静默跳过截图、用"此处应有截图"占位、或用文字描述代替。
- 用户不提供截图 → 报告不满足截图铁律，明确告知"缺真实截图的报告大概率被平台驳回"，由用户决定是否继续，不偷偷降级成无图报告。
